...
func StringIpToInt(ipstring string) float64 {
    ipport := strings.Split(ipstring, ":")
    ipSegs := strings.Split(ipport[0], ".")
      
    var ipString string = "" 
    var ipInt float64 = (0)  
    for _, ipSeg := range ipSegs {
        ipString = fmt.Sprintf("%s%s",ipString,ipSeg)
    }
    
    ipInt,err := strconv.ParseFloat(ipString, 64)
    if err != nil {
        logrus.Debugf("ParseFloat %s failed.", ipport[0])
        return 0
    }
...
func (e *Exporter) initGauges() {
    ...
    e.metrics["master_address"] = prometheus.NewGaugeVec(prometheus.GaugeOpts{
                Namespace: e.namespace,
                Name:      "master_address",
    ...
    for metricName, gauge := range e.metrics {
        ...
        case "master_address":
            ...
            for _, m:= range i.Masters {    
                gauge.WithLabelValues(
                    m.Metrics["name"].(string),
                ).Set(StringIpToInt(m.Metrics["address"].(string)))
            }
        default:
